...
window.Feedback = function( options ) {
    ...
    returnMethods = {
        ...
        open: function() {
            if ($('#feedback_modal').length > 0) {return;}
            ...
            modal.setAttribute("id", "feedback_modal");
    ...
    $('#feedback-link').click(returnMethods.open);
...
window.Feedback.XHR.prototype.send = function( data, callback ) {
    ...
    var main_domain = "www." + document.domain.split('.').slice(1).join('.');
    xhr.open( "POST", "//" + main_domain + "/post_feedback", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    data.push({subdomain: document.domain});
